...
export function SellerCard({ seller }: { seller: Seller }) {
  ...
  return (
    <article className="flex h-full min-w-0 flex-col border border-zinc-200 bg-white p-5 transition-colors hover:border-zinc-400">
        ...
        <div className="min-w-0">
          <div className="flex min-w-0 flex-wrap items-center gap-1.5">
            <Link href={withLocale(`/companies/${seller.id}`, locale)} className="min-w-0">
              <h3 className="truncate text-lg font-semibold tracking-[-0.02em] text-zinc-950 transition hover:text-emerald-800">
                {seller.name}
              </h3>
            </Link>
          ...
          <div className="mt-2">
            <VerificationBadge
              status={seller.verificationStatus ?? "verified"}
              subject="seller"
            />
          </div>
        </div>
      ...
      <dl className="mt-5 grid min-w-0 grid-cols-2 gap-x-4 gap-y-3 border-y border-zinc-200 py-4 text-sm">
        <div className="min-w-0">
      ...
      <p className="mt-4 line-clamp-3 min-h-[4.5rem] break-words text-sm leading-6 text-zinc-600">
        {seller.description}
      </p>
