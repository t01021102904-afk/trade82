...
export function AdminPayoutManagement({ selectedId }: { selectedId?: string }) {
  ...
  const [partnerPreparing, setPartnerPreparing] = useState(false);
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter((item) => {
      ...
      const partnerStatus = item.partnerPayout?.status ?? (item.partnerPreparationState === "NOT_PREPARED" ? "NOT_PREPARED" : "NOT_PRESENT");
      const hasPartner = item.hasPartnerAttribution;
  ...
  async function preparePartnerPayout() {
    const settlementLegId = selectedTransaction?.partnerSettlementLegId;
    if (!settlementLegId || selectedTransaction?.partnerPayout) return;
    setPartnerPreparing(true);
    setError("");
    try {
      const response = await fetch("/api/admin/payouts/reconcile", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ settlementLegId }),
      });
      const data = await response.json().catch(() => null);
      if (!response.ok || data?.ok !== true) throw new Error(t("payouts.reconcilePayoutError"));
      await load();
    } catch {
      setError(t("payouts.reconcilePayoutError"));
    } finally {
      setPartnerPreparing(false);
    }
  }
  ...
  const partnerStatus = selectedTransaction?.partnerPayout?.status ?? (selectedTransaction?.partnerPreparationState === "NOT_PREPARED" ? "NOT_PREPARED" : "NOT_PRESENT");
  const selectedPartnerPayout = selectedTransaction?.partnerPayout;
  return (
    <section className="admin-design-v2 grid min-w-0 max-w-full gap-4 overflow-x-hidden">
      ...
      <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(220px,0.36fr)_minmax(0,1fr)]">
        <nav className={mobileDetailOpen ? "hidden content-start gap-2 lg:grid" : "grid content-start gap-2"} aria-label={t("payouts.reviewQueue")}>
          {filteredTransactions.map((item) => { const seller = item.sellerPayout?.status ?? "NOT_PREPARED"; const partner = item.partnerPayout?.status ?? (item.partnerPreparationState === "NOT_PREPARED" ? "NOT_PREPARED" : "NOT_PRESENT"); return <button key={item.orderId} type="button" onClick={() => selectTransaction(item.orderId)} className={`grid min-w-0 gap-1 rounded-lg border bg-white p-3 text-left transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-600 dark:bg-zinc-950 ${selectedTransaction?.orderId === item.orderId ? "border-emerald-500 ring-2 ring-emerald-500" : ""}`}><span className="truncate text-xs font-semibold theme-foreground">{item.orderNumber}</span><span className="truncate text-xs theme-muted">{item.buyer.company}</span><span className="flex min-w-0 flex-wrap items-center justify-between gap-2 text-xs"><span className="truncate tabular-nums theme-muted">{formatTradeMoney(item.transaction.grossAmount, item.transaction.currency, locale)}</span><span className="flex min-w-0 flex-wrap gap-1"><span className="rounded-full border px-2 py-0.5 theme-muted">{t("payouts.sellerStatus")}: {reviewPayoutStatusLabel(seller, t)}</span>{item.hasPartnerAttribution ? <span className="rounded-full border border-sky-300 px-2 py-0.5 text-sky-800">{t("payouts.partnerStatus")}: {reviewPayoutStatusLabel(partner, t)}</span> : null}{item.warnings.length ? <span className="rounded-full border border-amber-300 px-2 py-0.5 text-amber-900">{t("payouts.warningBadge")}</span> : null}</span></span></button>; })}
        ...
        <div className={mobileDetailOpen ? "min-w-0" : "hidden min-w-0 lg:block"}>{!selectedTransaction ? <p className="rounded-xl border bg-white p-6 text-sm theme-muted dark:bg-zinc-950">{t("payouts.selectTransaction")}</p> : <article className="grid min-w-0 gap-5 rounded-xl border bg-white p-4 text-sm shadow-sm dark:bg-zinc-950 sm:p-5">
          ...
          {selectedTransaction.hasPartnerAttribution ? <DetailSection title={t("payouts.partnerPayoutDetails")}><Metric label={t("payouts.attribution")} value={selectedTransaction.partnerAttributionId ?? "—"} /><Metric label={t("payouts.partnerCommission")} value={formatTradeMoney(selectedPartnerPayout?.originalCommissionAmount ?? selectedTransaction.partnerExpectedCommissionAmount, selectedPartnerPayout?.currency ?? selectedTransaction.transaction.currency, locale)} /><Metric label={t("payouts.status")} value={selectedPartnerPayout ? payoutStatusLabel(selectedPartnerPayout.status, t) : reviewPayoutStatusLabel("NOT_PREPARED", t)} />{selectedPartnerPayout ? <><Metric label={t("payouts.partnerLegalName")} value={selectedPartnerPayout.partnerLegalNameSnapshot ?? "—"} /><Metric label={t("payouts.partnerDisplayName")} value={selectedPartnerPayout.partnerDisplayNameSnapshot ?? "—"} /><Metric label={t("payouts.company")} value={selectedPartnerPayout.partnerOrganizationSnapshot ?? "—"} /><Metric label={t("payouts.email")} value={selectedPartnerPayout.partnerEmailSnapshot ?? "—"} /><Metric label={t("payouts.contact")} value={selectedPartnerPayout.partnerPhoneSnapshot ?? "—"} /><Metric label={t("payouts.partnerStatus")} value={selectedPartnerPayout.partnerStatus} /><Metric label={t("payouts.payoutProfileStatus")} value={selectedPartnerPayout.payoutProfileStatus ?? "—"} /><Metric label={t("payouts.accountCountry")} value={selectedPartnerPayout.accountCountrySnapshot ?? "—"} /><Metric label={t("payouts.country")} value={selectedPartnerPayout.partnerResidenceCountrySnapshot ?? "—"} /><Metric label={t("payouts.accountHolder")} value={selectedPartnerPayout.accountHolderSnapshot ?? "—"} /><Metric label={t("payouts.bank")} value={selectedPartnerPayout.bankNameSnapshot ?? "—"} /><Metric label={t("payouts.maskedAccount")} value={selectedPartnerPayout.accountNumberMasked ?? (selectedPartnerPayout.accountNumberLast4 ? `•••• ${selectedPartnerPayout.accountNumberLast4}` : "—")} /><Metric label={t("payouts.payoutCurrency")} value={selectedPartnerPayout.payoutCurrencySnapshot ?? selectedPartnerPayout.currency} /><Metric label={t("payouts.payoutNumber")} value={selectedPartnerPayout.payoutNumber} /><Metric label={t("payouts.refundAdjustment")} value={formatTradeMoney(selectedPartnerPayout.reversalAdjustmentAmount, selectedPartnerPayout.currency, locale)} /><Metric label={t("payouts.finalCommission")} value={formatTradeMoney(selectedPartnerPayout.finalPayoutAmount, selectedPartnerPayout.currency, locale)} /><Metric label={t("payouts.holdUntil")} value={formatTradeDateTime(selectedPartnerPayout.holdUntil, locale)} /><Metric label={t("payouts.snapshotCapturedAt")} value={selectedPartnerPayout.snapshotCapturedAt ? formatTradeDateTime(selectedPartnerPayout.snapshotCapturedAt, locale) : "—"} /><Metric label={t("payouts.sentAt")} value={selectedPartnerPayout.sentAt ? formatTradeDateTime(selectedPartnerPayout.sentAt, locale) : "—"} /><Metric label={t("payouts.externalTransferReference")} value={selectedPartnerPayout.externalTransferReference ?? "—"} /><Metric label={t("payouts.externalBankReference")} value={selectedPartnerPayout.externalBankReference ?? "—"} /></> : null}</DetailSection> : null}
          ...
          {selectedPartnerPayout ? <section className="grid gap-3 rounded-lg border bg-white p-4 text-zinc-900 shadow-sm dark:bg-zinc-950 dark:text-zinc-100"><h3 className="text-sm font-semibold">{t("payouts.partnerActions")}</h3>{renderPartnerActions(selectedPartnerPayout)}</section> : selectedTransaction.hasPartnerAttribution ? <section className="grid gap-3 rounded-lg border bg-white p-4 text-zinc-900 shadow-sm dark:bg-zinc-950 dark:text-zinc-100"><h3 className="text-sm font-semibold">{t("payouts.partnerActions")}</h3><p className="text-sm theme-muted">{t("payouts.partnerPayoutNotPrepared")}</p><button type="button" onClick={() => void preparePartnerPayout()} disabled={partnerPreparing || !selectedTransaction.partnerSettlementLegId} className="inline-flex h-9 w-fit items-center gap-2 rounded-md bg-zinc-950 px-3 text-xs font-semibold text-white disabled:opacity-50">{partnerPreparing ? t("payouts.preparingPartnerPayout") : t("payouts.preparePartnerPayout")}</button></section> : null}
