...
export function AdminVerifications() {
  ...
  return (
    <div className="grid gap-8">
      ...
      <section>
        <h2 className="text-xl font-semibold text-zinc-950">Company verification requests</h2>
        <div className="mt-4 grid gap-4">
          {requests.map((item) => (
            <article key={item.id} className="grid gap-5 rounded-lg border border-zinc-200 bg-white p-5 lg:grid-cols-[1fr_auto]">
              ...
              <div className="flex gap-2"><button onClick={() => void reviewCompany(item.id, "verified")} className="rounded-md bg-emerald-700 px-3 py-2 text-sm font-medium text-white">Verify</button><button onClick={() => void reviewCompany(item.id, "rejected")} className="rounded-md border border-red-200 px-3 py-2 text-sm font-medium text-red-700">Reject</button></div>
            </article>
          ))}
      ...
      <section>
        <h2 className="text-xl font-semibold text-zinc-950">Pending verified-deal reviews</h2>
        <div className="mt-4 grid gap-4">
