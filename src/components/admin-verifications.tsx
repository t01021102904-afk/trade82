    ...
    <div className="grid gap-6">
      ...
      <section>
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-zinc-950">{admin.companyRequests}</h2>
          ...
          {filteredRequests.map((item) => {
            ...
            return (
              <button
                ...
                className="grid gap-4 rounded-md border border-zinc-200 bg-white p-4 text-left transition hover:border-blue-200 hover:shadow-sm lg:grid-cols-[minmax(0,1fr)_auto]"
              >
                ...
                <span className="self-start rounded-md border border-zinc-200 px-2.5 py-1.5 text-center text-xs font-medium text-zinc-700">
                  {admin.reviewDetails}
                </span>
              </button>
            );
          ...
          {!filteredRequests.length ? (
            <div className="rounded-md border border-dashed border-zinc-300 bg-white p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                <a href={withLocale("/admin/companies", locale)} className="inline-flex h-8 items-center rounded-md border border-zinc-200 px-2.5 text-xs font-medium text-zinc-700 hover:border-blue-200 hover:text-blue-700">{admin.viewAllCompanies}</a>
                <a href={withLocale("/admin", locale)} className="inline-flex h-8 items-center rounded-md border border-zinc-200 px-2.5 text-xs font-medium text-zinc-700 hover:border-blue-200 hover:text-blue-700">{admin.goToAdmin}</a>
              </div>
      ...
      <section>
        <h2 className="text-lg font-semibold text-zinc-950">{admin.companyReviewsTitle}</h2>
        <div className="mt-3 grid gap-2">
          ...
          {!companyReviews.length ? (
            <p className="rounded-md border border-dashed border-zinc-300 bg-white p-4 text-sm text-zinc-600">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-zinc-200 p-4">
          <div className="flex min-w-0 gap-3">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap gap-2">
                <h3 className="break-words text-lg font-semibold text-zinc-950">{company.tradeName || company.legalName}</h3>
                <Badge tone={tone}>{label}</Badge>
          ...
          <button
            type="button"
            onClick={onClose}
            className="h-8 rounded-md border border-zinc-200 px-2.5 text-xs font-medium text-zinc-700 hover:border-blue-200 hover:text-blue-700"
          >
            {admin.backToList}
          </button>
        </div>

        <div className="grid gap-5 p-4 lg:grid-cols-[1.35fr_0.65fr]">
          ...
          <aside className="grid content-start gap-4">
            <div className="rounded-md border border-zinc-200 bg-zinc-50 p-3">
              ...
              {request.documentFilename ? (
                <button
                  ...
                  className="mt-3 h-8 w-full rounded-md border border-zinc-200 bg-white px-2.5 text-xs font-medium text-zinc-700 hover:border-blue-200 hover:text-blue-700"
                >
                  {admin.openDocument}
                </button>
              ) : null}
            ...
            {actionState?.error ? (
              <p className="rounded-md border border-red-200 bg-red-50 p-2.5 text-sm text-red-700">{actionState.error}</p>
            ) : null}
