...
export function CompanyReviewsSection({
  ...
  async function submitReview() {
    ...
    const response = await fetch("/api/company-reviews", {
      ...
      body: JSON.stringify({ reviewedCompanyId: companyId, comment }),
    });
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-zinc-950">
            {companyRole === "seller"
              ? t("reviews.buyerFeedback")
              : t("reviews.sellerReviews")}
          </h2>
          {loading ? <p className="mt-1 text-sm text-zinc-500">{t("common.loading")}</p> : null}
        </div>
        ...
        {reviews.map((review) => (
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="text-xs text-zinc-500">{t("reviews.anonymous")}</span>
            </div>
