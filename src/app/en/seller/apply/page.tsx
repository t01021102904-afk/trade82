...
import { SupplierApplicationsUnavailable } from "@/components/supplier-applications-unavailable";
import { getCurrentSupplierApplication } from "@/lib/supplier-application";
import { supplierApplicationsEnabled } from "@/lib/supplier-application-feature";
import { requireSupplierApplicant } from "@/lib/require-auth";

export default async function EnSupplierApplyPage() {
  const user = await requireSupplierApplicant("/en/seller/apply");
  if (!supplierApplicationsEnabled())
    return <SupplierApplicationsUnavailable locale="en" />;
