  ...
  return (
    <div className="bg-white">
        ...
        <div className="mx-auto grid min-h-[560px] max-w-7xl content-center px-4 py-20 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link href={withLocale("/marketplace", "ko")} className="inline-flex items-center justify-center rounded-md bg-white px-5 py-3 text-sm font-semibold text-zinc-950 transition hover:bg-blue-50 hover:text-blue-700">
                {messages.common.browseProducts}
              </Link>
              <Link href={withLocale("/sellers", "ko")} className="inline-flex items-center justify-center rounded-md border border-white/30 px-5 py-3 text-sm font-semibold text-white transition hover:border-white hover:bg-white/10">
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-16 sm:px-6 lg:px-8">
        <SectionHeader label={messages.home.featuredProducts} title={messages.home.catalogPreview} description={messages.home.catalogDescription} action={<Link href={withLocale("/marketplace", "ko")} className="text-sm font-semibold text-blue-700">{messages.home.viewAllProducts}</Link>} />
      ...
      <section className="bg-zinc-50">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-16 sm:px-6 lg:px-8">
          <SectionHeader label={messages.home.featuredSellers} title={messages.home.sellerPreview} description={messages.home.sellerDescription} action={<Link href={withLocale("/sellers", "ko")} className="text-sm font-semibold text-blue-700">{messages.home.viewAllSellers}</Link>} />
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {featuredSellers.map((seller) => <SellerCard key={seller.id} seller={seller} />)}
          </div>
        </div>
      </section>
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <SectionHeader label={messages.home.howItWorks} title={messages.home.howTitle} description={messages.home.howDescription} />
        <div className="mt-8 grid gap-5 lg:grid-cols-3">
          {steps.map((step, index) => (
            <div key={step} className="rounded-lg border border-zinc-200 bg-white p-6">
              <span className="flex size-9 items-center justify-center rounded-md bg-blue-50 text-sm font-semibold text-blue-700">{index + 1}</span>
              <h3 className="mt-5 text-lg font-semibold text-zinc-950">{step}</h3>
              <p className="mt-3 text-sm leading-6 text-zinc-600">{stepText[index]}</p>
            </div>
          ))}
        </div>
      </section>
      <section className="border-t border-zinc-200 bg-zinc-950">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-14 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div>
            <h2 className="text-3xl font-semibold text-white">{messages.home.ctaTitle}</h2>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-300">{messages.home.ctaText}</p>
          </div>
          <Link href={withLocale("/onboarding/buyer", "ko")} className="inline-flex items-center justify-center rounded-md bg-white px-5 py-3 text-sm font-semibold text-zinc-950 transition hover:bg-blue-50 hover:text-blue-700">
