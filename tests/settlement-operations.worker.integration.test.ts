...
test("retry timing and terminal/manual-review records do not make Stripe calls", async () => {
  await envWithModes(async () => {
    ...
    assert.equal(result.manualReviewCount, 1);
    assert.equal(result.status, "FAILED");
    const retryNotDueState = await db.settlementLeg.findUnique({ where: { id: retryNotDue.leg.id }, select: { status: true, manualReviewRequired: true } });
    const manualReviewState = await db.settlementLeg.findUnique({ where: { id: manualReview.leg.id }, select: { status: true, manualReviewRequired: true } });
    const exhaustedState = await db.settlementLeg.findUnique({ where: { id: exhausted.leg.id }, select: { status: true, manualReviewRequired: true } });
    assert.deepEqual(retryNotDueState, { status: "READY", manualReviewRequired: false });
    assert.deepEqual(manualReviewState, { status: "READY", manualReviewRequired: true });
    assert.deepEqual(exhaustedState, { status: "READY", manualReviewRequired: true });
  });
});
