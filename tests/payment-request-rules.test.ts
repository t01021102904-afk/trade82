...
const feature = await import(
  new URL("../src/lib/message-payment-feature.ts", import.meta.url).href,
);
const {
...
const {
  getMessagePaymentRequestMode,
  isMessagePaymentFeatureEnabledForUser,
  loadOptionalMessagePaymentData,
} = feature;
const [webhookRouteSource, inquiriesRouteSource] = await Promise.all([
  readFile(new URL("../src/app/api/stripe/webhook/route.ts", import.meta.url), "utf8"),
  readFile(new URL("../src/app/api/inquiries/route.ts", import.meta.url), "utf8"),
]);

test("message payment mode is fail-closed when missing or invalid", () => {
  assert.equal(getMessagePaymentRequestMode({}), "off");
  assert.equal(
    getMessagePaymentRequestMode({ MESSAGE_PAYMENT_REQUEST_MODE: "unexpected" }),
    "off",
  );
  assert.equal(
    isMessagePaymentFeatureEnabledForUser("user_seller", {
      MESSAGE_PAYMENT_REQUEST_MODE: "off",
    }),
    false,
  );
});

test("off mode blocks seller payment creation and buyer Checkout before database payment access", async () => {
  const environment = { MESSAGE_PAYMENT_REQUEST_MODE: "off" } as const;
  assert.equal(isMessagePaymentFeatureEnabledForUser("user_seller", environment), false);
  assert.equal(isMessagePaymentFeatureEnabledForUser("user_buyer", environment), false);

  let paymentTableLoads = 0;
  const loaded = await loadOptionalMessagePaymentData({
    enabled: isMessagePaymentFeatureEnabledForUser("user_seller", environment),
    load: async () => {
      paymentTableLoads += 1;
      return ["payment-row"];
    },
  ...
  assert.equal(paymentTableLoads, 0);
});

test("internal mode permits only explicitly allowlisted Clerk users", () => {
  const environment = {
    MESSAGE_PAYMENT_REQUEST_MODE: "internal",
    MESSAGE_PAYMENT_INTERNAL_USER_IDS: "user_seller, user_buyer",
  } as const;
  assert.equal(isMessagePaymentFeatureEnabledForUser("user_seller", environment), true);
  assert.equal(isMessagePaymentFeatureEnabledForUser("user_buyer", environment), true);
  assert.equal(isMessagePaymentFeatureEnabledForUser("user_other", environment), false);
});

test("on mode preserves the existing role-specific route authorization checks", () => {
  const environment = { MESSAGE_PAYMENT_REQUEST_MODE: "on" } as const;
  assert.equal(isMessagePaymentFeatureEnabledForUser("user_seller", environment), true);
  assert.equal(isMessagePaymentFeatureEnabledForUser("user_buyer", environment), true);
});

test("payment-card load failures do not erase ordinary conversation data", async () => {
  const inquiries = [{ id: "inquiry_123", message: "Normal message data" }];
  let optionalLoadErrors = 0;
  const paymentRows = await loadOptionalMessagePaymentData({
    enabled: true,
    load: async () => {
      throw new Error("payment table unavailable");
    },
    onError: () => {
      optionalLoadErrors += 1;
    },
  });

  assert.equal(paymentRows, null);
  assert.equal(optionalLoadErrors, 1);
  assert.deepEqual(inquiries, [{ id: "inquiry_123", message: "Normal message data" }]);
});

test("webhook processing is never controlled by the request-payment rollout mode", () => {
  assert.doesNotMatch(
    webhookRouteSource,
    /isMessagePaymentFeatureEnabledForUser|getMessagePaymentFeatureState/,
  );
});

test("Contact Seller does not include payment tables in its conversation transaction", () => {
  const contactTransactionSource = inquiriesRouteSource.slice(
    inquiriesRouteSource.indexOf("async function findOrCreateConversationForParticipants"),
    inquiriesRouteSource.indexOf("export async function POST"),
  );
  assert.doesNotMatch(contactTransactionSource, /paymentRequests/);
});
