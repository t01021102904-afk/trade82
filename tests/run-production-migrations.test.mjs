...
test("the approved batch is followed by operations, analytics, partner payout, activation, and deferred application migrations", () => {
  ...
  assert.equal(localMigrations.at(-9), MERCHANT_MIGRATION);
  assert.equal(localMigrations.at(-8), OPERATIONS_MIGRATION);
  assert.equal(localMigrations.at(-7), ANALYTICS_MIGRATION);
  assert.equal(localMigrations.at(-6), PARTNER_PAYOUT_MIGRATION);
  assert.equal(localMigrations.at(-5), PARTNER_ACTIVATION_MIGRATION);
  assert.equal(localMigrations.at(-4), DEFERRED_APPLICATION_MIGRATIONS[0]);
  assert.equal(localMigrations.at(-3), DEFERRED_APPLICATION_MIGRATIONS[1]);
  assert.equal(localMigrations.at(-2), DEFERRED_APPLICATION_MIGRATIONS[2]);
